...
def scraping_character_profile(soup, character_info):
    fullSizeImage_url = (
        soup.select_one(".ak-character-picture > div")
        .get("style")
        .split(")")[0]
        .split("(")[-1]
    )
    avatarImage_url = (
        soup.select_one(".ak-directories-icon > div")
        .get("style")
        .split(")")[0]
        .split("(")[-1]
    )
    ...
    class_name = (
        soup.select_one(".ak-directories-breed")
        .getText()
        .replace(" ", "")
        .replace("\n", "")
    )
    ...
    for profession in professions:
        ...
        name_prof = (
            profession.select_one(".ak-title > a")
            .getText()
            .replace(" ", "")
            .replace("\n", "")
        )
...
def scraping_character_characteristics_profile(soup, character_info):
    list_characteristics_divs = soup.select(
        "div.ak-container.ak-panel.ak-caracteristics-details > div.ak-panel-content > div.ak-caracteristics-table-container.row"
    )
...
def get_character_info():
    ...
    if (
        result_request.get("login") == ""
        or result_request.get("password") == ""
        or result_request.get("character") == ""
    ):
        return "blank"
    ...
    names_founded = search_soup.select(
        ".ak-responsivetable > tbody > tr > td:nth-child(2) > a"
    )
    character_url = None

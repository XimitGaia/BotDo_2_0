...
class BinReader:
    def __init__(self, stream):
        ...
        while self._counter < self._total:
            vertex_from = self.add_vertex(
                self._bin_file_binary.read_double(), self._bin_file_binary.read_int32()
            )
            vertex_to = self.add_vertex(
                self._bin_file_binary.read_double(), self._bin_file_binary.read_int32()
            )
    ...
    def add_edge(self, vertex_from, vertex_to):
        ...
        if edge:
            return edge
        if not self.does_vertex_exist(vertex_from) or not self.does_vertex_exist(
            vertex_to
        ):
            return None
    ...
    def add_vertex(self, map_id: int, zone_id: int):
        ...
        if not vertice:
            self._vertex_uid += 1
            vertex = dict(
                {"map_id": map_id, "zone_id": zone_id, "u_id": self._vertex_uid}
            )

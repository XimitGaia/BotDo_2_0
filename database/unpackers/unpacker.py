...
class Unpacker:
    ...
    def d2o_dump(file_path, output_path):
        ...
        with open(
            f'{output_path}{os.sep}{file_name.replace("d2o", "json")}', "w"
        ) as json_output:
    ...
    def d2i_dump(file_path, output_path):
        ...
        file_name = os.path.basename(file_path)
        with open(
            f'{output_path}{os.sep}{file_name.replace("d2i", "json")}',
            "w",
            encoding="utf-8",
        ) as json_output:
    ...
    def ele_dump(file_path, output_path):
        ...
        with open(
            f'{output_path}{os.sep}{file_name.replace(".ele", ".json")}', "w"
        ) as json_output:
    ...
    def dlm_dump(file_path, output_path):
        ...
        with open(
            f'{output_path}{os.sep}{file_name.replace("dlm", "json")}', "w"
        ) as json_output:
            json.dump(data, json_output, indent=2)
            ...
            for name, specs in d2p_reader.files.items():
                ...
                try:
                    ...
                    if "swl" in name:
                        ...
                        with open(
                            f'{output_path}{os.sep}{name.replace("swl", "json")}', "w"
                        ) as json_output:
                            json.dump(swl_data, json_output, indent=4)
                        with open(
                            f'{output_path}{os.sep}{name.replace("swl", "swf")}', "wb"
                        ) as swf_output:
                        ...
                        data = dlm.read()
                        with open(
                            f'{output_path}{os.sep}{name.replace("dlm", "json")}', "w"
                        ) as json_output:
                            json.dump(data, json_output, indent=2)
                    else:
    ...
    def bin_dump(file_path, output_path):
        ...
        with open(
            f'{output_path}{os.sep}{file_name.replace("bin", "json")}', "w"
        ) as json_output:
            json.dump(data, json_output, indent=2)
    ...
    def dump_dofus_maps():
        ...
        for file_name in os.listdir(maps_path):
            ...
            if file_name.endswith(".d2p"):
                Unpacker.d2p_dump(
                    f"{maps_path}{os.sep}{file_name}", f"{output_base_path}{os.sep}maps"
                )

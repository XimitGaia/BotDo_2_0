...
class Ferramentas:
    ...
    def get_area_info():
        ...
        Ferramentas.print_a_retangle(
            (X, Y), (abs(point_2[0] - point_1[0]), abs(point_2[1] - point_1[1]))
        )
        print(
            "X:",
            X,
    ...
    def show_matches_on_screen(image):
        ...
        for match in matches_position:
            ...
            Ferramentas.print_a_retangle(
                ...
                (image_position[2], image_position[3]),
            )
        time.sleep(3)
    ...
    def get_image_first_info(image):
        ...
        return {
            "whidth": picture.shape[1],
            "high": picture.shape[0],
            "pixel_number": picture.size,
        }

    @staticmethod
    def resize_untill_find(
        image, bigger_or_smaller, min_size=50, max_size=150, one_match_per_point="on"
    ):  # (imagem, diminuir ou aumentar, min_size = reduzir ate X%, 30 significa reduzir 70% do tam original, max_size = aumentar até X%)
        ...
        if bigger_or_smaller == "smaller":
            while int((high / image_info["high"]) * 100) >= int(min_size):
                ...
                width = round(
                    image_info["whidth"] - ((image_info["high"] - high) * image_rate)
                )
                dimension = (width, high)  # nova dimensao da imagem
                # print('Dimension:', dimension)
                image_resized = cv2.resize(
                    image, dimension
                )  # muda tamanho apra dimensao nova
                if one_match_per_point == "off":
                    matches_position = pyautogui.locateAllOnScreen(
                        image_resized, confidence=0.99
                    )  # lista todos os matches 90% iguais ao da imagem redimensionada
                    for region in matches_position:
                        region = list(region)
                        ocurrencies[
                            "Match_{}".format(match_number)
                        ] = region  # region = [X,Y, new_Width , new_High]
                        match_number += 1
                if one_match_per_point == "on":
                    match_position = pyautogui.locateOnScreen(
                        image_resized, confidence=0.99
                    )
                    if match_position != None:
                        ...
                        ocurrencies[
                            "Match_{}".format(match_number)
                        ] = match_position  # region = [X,Y, new_Width , new_High]
                        match_number += 1
                high -= 1
        if bigger_or_smaller == "bigger":
            while int((high / image_info["high"]) * 100) <= int(max_size):
                print("{}%".format(round((high / image_info["high"]) * 100)))
                width = round(
                    image_info["whidth"] + ((high - image_info["high"]) * image_rate)
                )
                dimension = (width, high)  # nova dimensao da imagem
                print("Dimension:", dimension)
                image_resized = cv2.resize(
    ...
    @staticmethod
    def print_ocurrencies_on_screen(
        image, big_or_small, min_size=50, max_size=150, one_match_per_point="on"
    ):
        match_list = Ferramentas.resize_untill_find(
            image,
            big_or_small,
            min_size=min_size,
            max_size=max_size,
            one_match_per_point="on",
        )
        for region in match_list:
            Ferramentas.print_a_retangle(
                (match_list[region][0], match_list[region][1]),
                (match_list[region][2], match_list[region][3]),
            )
...
while True:
    ...
    if option == 1:
        ...
        Ferramentas.resize_untill_find(
            image_path,
            reduce_or_expand,
            min_size=min_size,
            max_size=max_size,
            one_match_per_point=one_match_per_point,
        )
        ...
        max_size = input("max_size: ")
        one_match_per_point = input("one_match_per_point(on/off): ")
        Ferramentas.print_ocurrencies_on_screen(
            image_path,
            reduce_or_expand,
            min_size=min_size,
            max_size=max_size,

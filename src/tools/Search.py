...
class Search:
    ...
    def search_image(
        ...
        if type(screen) == tuple or screen == "":
            screen = ImageGrab.grab(
                screen
            )  # grab the screen ('' is fullscreen) or (x1,y1,x2,y2)
        else:
            # screen = screen
        ...
        reference_point = (
            0,
            0,
        )  # poin to use as reference when going trough the matrix
        ...
        for x in range(reference_match_area[0][0], reference_match_area[1][0] + 1):
            for y in range(
                reference_match_area[0][1], reference_match_area[1][1] + 1
            ):  # go trough each pixel
                ...
                for pos in validator_group:  # check some pixels of the image
                    ...
                    if not Search._check_color_renge(
                        ...
                        validator_error += 1
                        if validator_error >= round(
                            len(validator_group) * match_tolerance
                        ):  # if validation exeeds the number of non-validated pixels allowed it breaks
                            validated = False
                            break
    ...
    def _get_valid_pixels(
        image: PngImagePlugin.PngImageFile,
    ) -> list:  # get the pixels that has no transparency
        img = image
    ...
    @staticmethod
    def _get_validator_group(
        valid_pixels: list, validator_group_porcentage: float = 0.05
    ):  # get some piexels to pre-validate the image
    ...
    def _get_reference_match_area(
        reference_point: tuple, image_size: tuple, screen_size: tuple
    ...
    def _check_color_renge(
        ...
        Hdelta = round(
            color_tolerance * 42.5
        )  # 42.5 force the color to stay in its range (remember 255+x = x for x< 255, like a circle) red [234-21] yellow [21-64] green [64-106] cian[106-149] blue [149-191] cian [191-234]
        Hdist = abs(H1 - H2)
        if abs(S1 - S2) <= round(saturation_tolerance * 255):
            if abs(V1 - V2) <= round(bright_tolerance * 255):
                if (
                    255 - Hdelta <= Hdist or Hdist <= Hdelta
                ):  # check if the clors area in the delta range
                    return True
        return False
